...
def create_mat_lib(sim_run_folder_path, material, material_filename="material.cmx"):
    ...
    try:
        with open(material_file_path, "w", encoding="utf-8") as file:
            ...
            file.write(r'<Column name="Default Thickness" unit="um" />	</Material>' + "\n")
            file.write(r"<Metal>" + "\n")
...
def generate_dns_list(sim_run_folder_path, brd_file_path, extracta_path, DNS_filter_list):
    """generated DNS list"""
    ...
    process = subprocess.Popen(args)  # execute extracta.exe to generate a temp DNS list file
    process.wait()
    with open(temp_dns_file_path, encoding="utf-8") as f:
        data = f.readlines()

...
from PyCode.tab_writter import (
    summary_create,
    summary_update,
    tab_create_irdrop,
    tab_update_irdrop,
...
def simulation_setting_reader(
    ...
    enabled_indices = [index for index, value in enumerate(enabled_col) if value == "TRUE"]
        ...
        if unique_key:
            ...
            if workflow.lower() in multi_board_workflow:
                ...
                for i in range(start_row_index, end_row_index + 1):
                    ...
                    if brd_path_col[i]:
                        try:
                            brd_path_parts = brd_path_col[i].split(":", 1)  # split by the first :
                    ...
                    try:
                        # simulation options is optional
                        simulation_options[block_name] = simulation_options_col[i].strip()
                    except ValueError:
                        simulation_options[block_name] = ""

                if len(brd_path.keys()) < end_row_index + 1 - start_row_index:
                    msgbox(pwt_msgbox, f"Duplicate block names are found for {unique_key}!")
                    raise ValueError
...
def stackup_settings_reader(spreadsheet_url, stackup_tab_name):
    ...
    response2 = (
        service.spreadsheets()
        .values()
        .get(spreadsheetId=spreadsheet_id, majorDimension="ROWS", range=stackup_tab_name)
        .execute()
    )
    ...
    stackup = settings2[temp + 1 :]  # attention: there is an extra row under stackup now
...
def brd_result_writter(sim_info, pwt_msgbox):
    """
    spreadsheet_url, sink_path, sinkpin_path, tab_name, cell_color,
    merge_flag, call_time, file_num, occupied_row
    unit is mV
    ...
    if workflow.lower() == "Single_IR".lower():
        ...
        if os.path.exists(sinkpin_path):
            with open(sinkpin_path, encoding="utf-8") as csvfile:
                csv_data = csv.reader(csvfile, delimiter=",")
                for row in csv_data:
        ...
        try:
            tab_upload_resmeas(tab_name, service, spreadsheet_id, resis_path, pdn_report_url)
        except RuntimeError as e:
        ...
        for root, dirs, _ in os.walk(result_folder_path):
            for dir_name in dirs:
                if dir_name.endswith("_Result_Files"):
                    ...
                    sinkpin_path = os.path.join(root, dir_name, "CSVFolder\\Sinkpin.csv")
                    ...
                    if tab_name in sheet_title:
                        if os.path.exists(sink_path):
                            ...
                            msgbox(pwt_msgbox, f"result for {tab_name} is not uploaded!")

...
from PyCode import generate_tcl, google_io, sim_prepare
...
class MainGUI:
    ...
    def __init__(self, root):
        ...
        self.msg_box_display_logs("Current Auto powertree script is Ver." + self.script_ver)
    ...
    def create_pwt_tab(self):
        ...
        self.pwt_load_file_button.grid(row=0, column=0, padx=(10, 10), pady=10, sticky="w")
        # Create Entry box to show the setting filepath
        self.pwt_setting_path_box = ttk.Entry(self.frame_top, width=50)
        self.pwt_setting_path_box.grid(row=0, column=1, padx=(10, 10), pady=10, sticky="ew")
        ...
        self.pwt_apply_stackup.grid(row=1, column=0, padx=(10, 10), pady=(0, 10), sticky="w")
        ...
        self.frame_msgbox = tk.Frame(self.pwt_tab, height=300, borderwidth=2, relief="groove")
        self.frame_msgbox.grid(row=1, column=0, columnspan=2, padx=(5, 10), pady=10, sticky="nsew")
    ...
    def create_upload_tab(self):
        ...
        self.upload_setting_path_box.grid(row=0, column=1, padx=(10, 10), pady=10, sticky="ew")
        ...
        # 2. create a frame for displaying prj_setting details in msg box
        # this is in the middle of the window
        self.frame_msgbox = tk.Frame(self.upload_tab, height=300, borderwidth=2, relief="groove")
        self.frame_msgbox.grid(row=1, column=0, columnspan=2, padx=(5, 10), pady=10, sticky="nsew")
    ...
    def preprocessing(self):
        ...
        try:
            if not os.path.exists(self.sim_run_folder_path):
                ...
                self.msg_box_display_logs("Simulation run folder is created succesfully")
        except RuntimeError as e:
            self.msg_box_display_logs(f"Error creating folder: {str(e)}")
        ...
        if self.autopwt_settings["stackup_tab_name"]:
            ...
            self.msg_box_display_logs("Stackup info is missing and stack up will not be applied!")
        ...
        if sig_dir[-1] != "/":
            sig_dir = (
                "/".join(sig_dir.split("/")[:-1]) + "/" + self.autopwt_settings["SIG_VER"] + "/"
            )
        else:
            sig_dir = (
                "/".join(sig_dir.split("/")[:-2]) + "/" + self.autopwt_settings["SIG_VER"] + "/"
            )
        ...
        if return_code == 0:
            self.msg_box_display_logs("Pre-process of the brd file completed successfully")
        else:
    ...
    def pwt_load_setting_file(self):
        ...
        if self.autopwt_setting_path:
            ...
            with open(self.autopwt_setting_path, encoding="utf-8") as prj_json:
                try:
            ...
            self.msg_box_display_logs("PowerDC version is: " + self.autopwt_settings["SIG_VER"])
            ...
            if self.proj_main_path:
                self.msg_box_display_logs("PDN Project path is: \n" + self.proj_main_path)
            ...
            if self.num_of_simulations > 0:
                self.msg_box_display_logs(f"No of simulations enabled: {self.num_of_simulations}")
    ...
    def upload_load_setting_file(self):
        ...
        if self.autopwt_setting_path:
            ...
            with open(self.autopwt_setting_path, encoding="utf-8") as prj_json:
                try:
                    self.autopwt_settings = json.load(prj_json)
                    self.upload_msg_box_display_logs(f"{self.autopwt_setting_path} is read!")

                except FileNotFoundError as e:
                    self.upload_msg_box_display_logs(f"Error reading JSON file: {str(e)}")
            ...
            if self.upload_workflow.lower() not in defined_workflows:
                self.upload_msg_box_display_logs("Wrong work flow is defined. Please check")
                raise ValueError
            else:
                self.upload_msg_box_display_logs(f"{self.upload_workflow} result will be uploaded")
    ...
    def upload_result(self):
        ...
        if sim_info["report_in_PDF"].lower() == "yes":
            ...
            if pdf_files:
                ...
                try:
                    ...
                    self.upload_msg_box_display_logs("PDF report is uploaded to Google Drive")
                except RuntimeError as e:
                    self.upload_msg_box_display_logs(f"Error uploading PDF report: {str(e)}")
            ...
            # check if any htm files are there and convert to pdf file
                ...
                if htm_files:
                    ...
                    if self.htm2pdf_tool_path:
                        directory, file_name_with_extension = os.path.split(report_htm_path)
                        ...
                        try:
                            ...
                            self.upload_msg_box_display_logs("Simulation report file is generated")
                        except RuntimeError as e:
                            self.upload_msg_box_display_logs(f"Error creating PDF report: {str(e)}")
        ...
        try:
            ...
            self.upload_msg_box_display_logs(f"Error uploading result to Gsheet {str(e)}")
    ...
    def pwt_sim_start(self):
        ...
        for item in self.sim_list:
            ...
            if workflow in self.single_brd_workflow:
                sim_run_key_folder_path = os.path.join(self.sim_run_folder_path, item["unique_key"])
            ...
            if workflow in self.multi_brd_workflow:
                ...
                if self.sig_dir[-1] != "/":
                    self.sig_dir = "/".join(self.sig_dir.split("/")[:-1]) + "/" + sig_ver + "/"
                else:
                    self.sig_dir = "/".join(self.sig_dir.split("/")[:-2]) + "/" + sig_ver + "/"
                ...
                for key in block_keys:
                    ...
                    if simulation_brd_option:
                        ...
                        read_option_input_info["sim_run_key_folder_path"] = sim_run_key_folder_path
                ...
                if workflow.lower() == "multibrd_IR".lower():
                    ...
                    main_tcl_path = generate_tcl.generate_multibrd_limitedir_tcl(sim_info)
                    pdc_cmd = (
                        os.path.join(self.sig_dir, "tools/bin/powerdc.exe")
                        + ' -PSCelsiusDC_G -tcl "'
                        + main_tcl_path
        ...
        self.thread = threading.Thread(target=self.sequential_sim_run, args=(command_list,))
        self.thread.start()
        self.msg_box_display_logs(f"Simulation started! Total of simulations: {len(self.sim_list)}")
    ...
    def post_processing(self):
        ...
        for item in self.sim_list:
            workflow = item["workflow"].lower()
            if workflow in self.single_brd_workflow or workflow in self.multi_brd_workflow:
                sim_run_key_folder_path = os.path.join(self.sim_run_folder_path, item["unique_key"])
                if not os.path.exists(sim_run_key_folder_path):
                    os.makedirs(sim_run_key_folder_path)
                ...
                if os.path.exists(sim_run_key_folder_path):
                    ...
                    if folder_paths:
                        result_folder_path = max(folder_paths, key=lambda f: os.path.getctime(f))
                        print(f"The latest folder is: {os.path.basename(result_folder_path)}")
                    else:
